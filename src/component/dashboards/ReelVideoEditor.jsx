...
import { FaVideo, FaSave, FaTimes, FaPlay, FaDownload, FaTrash, FaLayerGroup, FaMusic, FaPlus, FaSpinner } from 'react-icons/fa';
...
const ReelVideoEditor = () => {
    ...
    const mergeVideosWithFFmpeg = async (mediaFiles, musicFiles, options, folder) => {
        try {
            ...
            const   payload = {
                mediaFiles: await Promise.all(mediaFiles.map(async (file, index) => {
                ...
                musicFiles: await Promise.all(musicFiles.map(async (file, index) => {
            ...
            if (response && response.data && response.headers['content-type'] && response.headers['content-type'].includes('video')) {
    ...
    return (
        <div className="flex flex-col min-h-screen bg-gradient-to-br from-gray-100 to-blue-100">
                ...
                <section className="w-full md:w-[420px] flex-shrink-0 flex flex-col gap-6">
                    {showTemplatePanel && (
                        <div className="bg-white rounded-2xl shadow-lg p-4 border border-blue-100">
                            ...
                            <ReelTemplatePanel key={templatePanelKey} onGenerate={handleTemplateGenerate} />
                        </div>
                    )}
                </section>
                            ...
                            <div className="relative aspect-[9/16] w-[240px] bg-gradient-to-br from-blue-100 to-blue-200 border-2 border-blue-200 rounded-3xl shadow-xl overflow-hidden flex items-center justify-center h-[420px] mx-auto p-0">
                                {mergedVideoUrl ? (
                                    ...
                                    <div className="flex flex-col items-center justify-center w-full h-96">
                                        ...
                                        <span className="text-blue-700 font-semibold">{mergeProgress || 'Merging video...'}</span>
                                    </div>
                                ) : (
                                    <div className="flex flex-col items-center justify-center w-full h-96 text-gray-400">
                                        <FaVideo className="text-6xl mb-4" />
                                        <span className="text-s font-medium ">Your preview will appear here</span>
                                    </div>
                                )}
                            </div>
                            {/* Divider */}
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 border border-blue-100 mt-6 max-w-7xl mx-auto w-full">
                <h2 className="text-xl font-bold text-blue-700 mb-6 flex items-center gap-2"><FaVideo /> Saved Reels</h2>
                {loadingVideos ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {savedVideos.map((video) => {
                            return (
                                <div key={video._id} className="bg-white rounded-lg shadow-md overflow-hidden border border-gray-100 transition-transform duration-200 hover:scale-105 hover:shadow-2xl cursor-pointer">
