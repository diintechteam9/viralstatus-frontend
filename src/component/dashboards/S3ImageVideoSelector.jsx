...
const S3ImageVideoSelector = ({ onClose, onFileSelect }) => {
    ...
    try {
      ...
      if (response.data && response.data.subcategories) {
        setCategories((prev) =>
          prev.map((c) =>
            c._id === categoryId
              ? { ...c, subcategories: response.data.subcategories }
              : c
          )
        );
  ...
  const fetchCategories = async () => {
    try {
      ...
      if (response.data && response.data.categories) {
        const initialCategories = response.data.categories.map((c) => ({
          ...c,
          subcategories: [],
        }));
        ...
        const categoriesWithSubcategories = await Promise.all(
          initialCategories.map(async (category) => ({
            ...category,
            subcategories: await fetchSubcategories(category._id),
          }))
        );
  ...
  const handleFileTypeChange = async (type) => {
    if (fileType !== type) {
      ...
      if (selectedFolderView)
        await fetchFilesForFolder(selectedFolderView, type);
    }
  };
  ...
  const toggleSubCategory = async (categoryId, subCategoryId) => {
    setSelectedSubCategoryView((prev) =>
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 text-gray-700 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100"
                >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {files.length === 0 ? (
                    <div className="col-span-full text-center text-gray-400 py-12">
                      ...
                      <p>No {fileType.toLowerCase()} files in this folder.</p>
                    </div>
                  ) : (
                    files.map((file) => (
                      <div
                        key={file._id}
                        className="bg-white rounded-xl shadow-lg p-4 flex flex-col items-center gap-4 group hover:shadow-2xl transition-shadow cursor-pointer"
                      ...
                      >
                        ...
                        <h3 className="text-md font-bold truncate text-center w-full">
                      ...
                      <div className="flex flex-wrap gap-4 mb-6">
                        {category.subcategories.map((subCat) => (
                          <div key={subCat._id} className="relative">
                            <div
                              className={`bg-gray-50 rounded-lg p-3 cursor-pointer border-2 transition-all ${
                          ...
                          <div className="flex flex-wrap gap-8 mt-4">
                            {folders.filter(
                              (f) =>
                                ...
                                f.subcategoryId === selectedSubCategoryView
                            ).length > 0 ? (
                              folders
                                .filter(
                                  (f) =>
                                ...
                                .map((folder) => (
                                  ...
                                  >
                                    <FaFolder className="text-blue-500 text-7xl mb-2 group-hover:scale-105" />
