...
const SocialMediaCard = ({ platform, icon: Icon, logoUrl, isConnected, profileData, loginUrl, onConnect, onDisconnect, uploadComponent }) => (
      ...
      {!isConnected ? (
        onConnect ? (
          <button onClick={onConnect} className="no-underline w-full block">
            <div className="border-2 border-blue-500 rounded p-3 text-center bg-white hover:bg-blue-50 transition-colors">
              <div className="flex items-center justify-center mb-2">
                <div className={`rounded-full flex items-center justify-center mr-2 ${platform === "Instagram" ? "bg-gradient-to-tr from-yellow-400 via-pink-500 to-purple-600" : "bg-red-500"}`} style={{ width: 48, height: 48 }}>
...
const AccountsTab = () => {
  ...
  const loadYoutubeData = async () => {
    try {
      ...
      const t = getToken();
      if (!uid || !t) return;
      const r = await fetch(`${API_BASE_URL}/auth/youtube/profile?userId=${uid}`, {
        headers: { Authorization: `Bearer ${t}` },
        credentials: "include",
      });
      if (r.ok) {
  ...
  const handleYouTubeDisconnect = async () => {
    try { await disconnectYoutube(); } catch (_) {}
    finally {
      setIsYouTubeAuthenticated(false);
      setYoutubeData(null);
    }
