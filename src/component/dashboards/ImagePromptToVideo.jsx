...
const ImagePromptToVideo = () => {
  ...
  const handleImageUpload = (idx, file) => {
    ...
    reader.onload = (e) => {
      ...
      setPromptRows(rows => rows.map((row, i) => i === idx ? { ...row, uploadedImage: dataUrl, error: '' } : row));
    };
  ...
  const generateVideo = async (idx) => {
    ...
    try {
      ...
      if (res.data.success && (res.data.videoUrl)) {
        setPromptRows(rows => rows.map((r, i) => i === idx ? { ...r, videos: [res.data.videoUrl], loading: false } : r));
      } else {
