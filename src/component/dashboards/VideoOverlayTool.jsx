...
import { FaUpload, FaTrash, FaVideo, FaDownload, FaCog, FaPlay, FaFilm, FaLayerGroup, FaClock, FaCloudUploadAlt } from 'react-icons/fa';
      ...
      <div className="flex flex-col items-center min-h-screen bg-gradient-to-br from-gray-100 to-blue-100 py-8 px-2">
        <div className="w-full max-w-5xl bg-white rounded-2xl shadow-2xl p-8 flex flex-col md:flex-row gap-8">
          ...
          <form onSubmit={handleSubmit} className="flex-1 flex flex-col gap-6">
            ...
            <p className="text-gray-500 mb-4">Easily overlay a video or image onto your main video. Choose position, start time, duration, and preview the result instantly.</p>
              ...
              {!mainVideo ? (
                <div className="border-2 border-dashed border-blue-300 rounded-lg p-4 text-center hover:bg-blue-50 transition">
                  <div className="flex justify-around items-center">
                    ...
                    />
                    <label htmlFor="main-video-upload" className="cursor-pointer flex flex-col items-center gap-2 p-4">
                      <FaUpload className="text-3xl text-blue-400" />
                      <span className="text-sm text-blue-600 font-medium">Upload from Computer</span>
                    </label>
                    ...
                    >
                      ...
                      <span className="text-sm text-blue-600 font-medium">Select from S3</span>
                    </button>
                  </div>
                </div>
              ) : (
                <div className="relative bg-white rounded-lg p-2 border border-blue-200 flex items-center gap-2">
                  <video src={URL.createObjectURL(mainVideo)} className="w-32 h-20 object-cover rounded shadow" controls />
                  <div className="flex-1">
                    <p className="text-sm font-medium text-blue-700 truncate">{mainVideo.name}</p>
                  </div>
                  <button
                    type="button"
            ...
            <div className="bg-gray-50 rounded-xl p-4 shadow-sm border border-gray-200">
              ...
              {!overlayFile ? (
                <div className="border-2 border-dashed border-blue-300 rounded-lg p-6 text-center hover:bg-blue-50 transition">
                  <div className="flex justify-around items-center">
                    <label htmlFor="overlay-file-upload" className="cursor-pointer flex flex-col items-center gap-2 p-4 rounded-lg hover:bg-blue-100">
                      <FaUpload className="text-3xl text-blue-400" />
                      <span className="text-sm text-blue-600 font-medium">Upload from Computer</span>
                    </label>
                    <input
                ...
                <div className="relative bg-white rounded-lg p-2 border border-blue-200 flex items-center gap-2">
                  {overlayFile.type.startsWith('video/') ? (
                    ...
                    <img src={URL.createObjectURL(overlayFile)} className="w-32 h-20 object-cover rounded shadow" alt="Overlay" />
                  )}
                  <div className="flex-1">
                    <p className="text-sm font-medium text-blue-700 truncate">{overlayFile.name}</p>
                    {overlayFile.type.startsWith('video/') && overlayVideoDuration > 0 && (
                      <p className="text-xs text-gray-500">Duration: {formatTime(overlayVideoDuration)}</p>
                    )}
                  </div>
                  <button
                    type="button"
            ...
            <div className="bg-gray-50 rounded-xl p-4 shadow-sm border border-gray-200 flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-4">
                <div className="flex flex-col md:flex-row gap-4">
                  <div className="flex-1">
                    <label className="block text-sm font-medium text-gray-700 mb-1">Overlay Start Time (seconds)</label>
                    <input
                      type="number"
                      min="0"
                      step="1"
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex items-center gap-4">
                    <input
                      ...
                      disabled={isProcessing}
                    />
                    {overlayFile && overlayFile.type.startsWith('video/') && overlayVideoDuration > 0 && (
                      <button
                        type="button"
                        onClick={() => setOverlayDuration(Math.round(overlayVideoDuration))}
                  ...
                  {overlayDuration > 0 && (
                    <p className="text-xs text-gray-500">
                      ...
                      {overlayFile && overlayFile.type.startsWith('video/') && overlayVideoDuration > 0 && 
                        ` (${overlayDuration >= overlayVideoDuration ? 'full video' : `${Math.round((overlayDuration / overlayVideoDuration) * 100)}% of video`})`
                      }
                    </p>
                  )}
                </div>
            ...
            <button
              type="submit"
              className={`w-full py-3 rounded-lg flex items-center justify-center gap-2 text-lg font-semibold shadow transition-all duration-200 ${(!mainVideo || !overlayFile || isProcessing || overlayStart < 0 || overlayDuration <= 0) ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700 text-white'}`}
              disabled={!mainVideo || !overlayFile || isProcessing || overlayStart < 0 || overlayDuration <= 0}
            >
              <FaLayerGroup />
          ...
          <div className="flex-1 flex flex-col items-center justify-center bg-gradient-to-br from-blue-50 to-white rounded-2xl shadow-inner p-6 min-h-[400px]">
            ...
            {processedVideoUrl ? (
              <>
                <video src={processedVideoUrl} className="w-full max-w-md h-64 object-contain rounded-xl border-2 border-blue-200 shadow-lg mb-4" controls />
                <button
                  type="button"
              ...
              <div className="flex flex-col items-center justify-center h-full w-full text-gray-400">
                ...
                <span className="text-lg font-medium">Your overlayed video will appear here</span>
              </div>
            )}
          </div>
        </div>
